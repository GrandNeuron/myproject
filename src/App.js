...
function App() {
  ...
  const handleEquals = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        response = await axios.patch(`http://localhost:8080/tasks/${editingId}`, {
          task: expression,
        });
      } else {
  ...
  // Function to handle edit (load task for editing)
  const handleEdit = (calc) => {
    setExpression(calc.task);
